...
impl Preview {
    pub fn new(display: GlutinFacade) -> Preview {
        Preview {
            display: display
        }
    }
    ...
    pub fn render(&self) {
        ...
        let vertex_buffer = glium::VertexBuffer::new(&self.display, &cube_vertices).unwrap();
        let indices = glium::index::IndexBuffer::new(&self.display,
        ...
        let program = glium::Program::from_source(&self.display, vertex_shader_src, frag_shader, None).unwrap();
